...
int main() {
    ...
    ofstream out;
    out.open("D:\\workspace\\clion-workspace\\algorithm-learn\\suffix_auto_machine\\path200.txt");
    while (in >> s) {
        in >> s;
        int len = s.size();
        int u = 0;
        vector<int> v, path,w,first;
        for (int i = 0; i < len; i++) {
            u = exSam.next[u][trans(s[i])];
            v.push_back(exSam.id[u]);
            w.push_back(exSam.firstpos[u] - len + 1);
        }
        ...
        if(cnt>=REPEATS_SIZE){
            path.push_back(v[v.size()-1]);
            first.push_back(w[w.size()-1]);
        }
        out << (path.size()>=1?path[0]:exSam.id[u]) << " " << (first.size()>=1?first[0]:exSam.firstpos[u] - len + 1) << " ";
